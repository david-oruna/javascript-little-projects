...
let cardSymbols = ["1","2","3","4", "5", "6","7","8","9"]
let visibleCard;
let firstCard;
let secondCard;
...
function generateGameBoard() {
    let cardItemList = shuffle([...cardSymbols, ...cardSymbols]); // Duplicating the array to create pairs
    ...
    cardItemList.forEach(function (cardClassName, index) {
        ...
        // Create the front side with the planet image
        let frontItem = document.createElement("div");
        frontItem.classList.add("front");

        let imageItem = document.createElement("img");
        imageItem.src = `planets/${cardClassName}.png`; // Use the corresponding image for the card
        imageItem.alt = `Card ${cardClassName}`; // Accessibility feature

        frontItem.appendChild(imageItem);

        // Create the back side
        let backItem = document.createElement("div");
        backItem.classList.add("back");
        backItem.style.backgroundImage = "url('planets/back.png')"; // Set the back image

        cardItem.appendChild(frontItem);
        cardItem.appendChild(backItem);
